...
import { GET } from "@/app/api/source/liked/route";
...
function buildRequest(cookies: string[] = []): NextRequest {
  return new NextRequest("http://localhost/api/source/liked?sourceProvider=spotify", {
    method: "GET",
    headers: cookies.length ? { cookie: cookies.join("; ") } : undefined
  });
}

describe("GET /api/source/liked", () => {
  beforeEach(() => {
  ...
  it("returns liked tracks for approved requests with a spotify session", async () => {
