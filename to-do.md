...
- 2026-02-15: Added duplicate playlist detection for Spotify→TIDAL transfers: new `listTidalUserPlaylists()` function fetches user's existing TIDAL playlists via `GET /playlists?filter[owners.id]`, new `GET /api/transfer/tidal-playlists` endpoint, client-side case-insensitive name matching during preview. Preview page shows "Already on TIDAL" amber info section listing matched playlists, "Allow duplicate playlists" checkbox to opt-in, dimmed playlist cards with "Skipped" badge for excluded playlists, and reactive hero card counts. Transfer execution filters out duplicate playlists unless checkbox is checked. Added `TidalExistingPlaylist` shared type, exported `getTidalUserId` from tidal-write.
