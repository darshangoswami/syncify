...
- Guarded routes (403 until approved):
  ...
  - `GET /api/transfer/tidal-playlists`
  - Returns existing TIDAL playlists for the authenticated user (used for duplicate detection).
...
- [x] Add proactive OAuth token refresh so long transfers survive past the 1-hour expiry window.
...
- 2026-02-11: Fixed transfer progress page scroll: changed container from `overflow-hidden` to `overflow-y-auto` so the Stop Transfer button and warning text are accessible on shorter screens (iOS Chrome).
- 2026-02-11: Added Screen Wake Lock API to prevent phone screen from turning off during preview fetching and transfer execution. Wake lock is acquired on start, released on completion/error/cancel/unmount, and re-acquired on `visibilitychange` when tab regains focus. Graceful degradation on unsupported browsers. Updated warning text on preview and progress screens to remind users to keep screen on.
- 2026-02-11: Removed overly aggressive global `prefers-reduced-motion` rule that was killing all animations (including transfer spinner) on some devices even without the accessibility setting enabled. Battery saver and certain browser defaults can trigger `prefers-reduced-motion: reduce`.
- 2026-02-15: Added duplicate playlist detection for Spotify→TIDAL transfers: new `listTidalUserPlaylists()` function fetches user's existing TIDAL playlists via `GET /playlists?filter[owners.id]`, new `GET /api/transfer/tidal-playlists` endpoint, client-side case-insensitive name matching during preview. Preview page shows "Already on TIDAL" amber info section listing matched playlists, "Allow duplicate playlists" checkbox to opt-in, dimmed playlist cards with "Skipped" badge for excluded playlists, and reactive hero card counts. Transfer execution filters out duplicate playlists unless checkbox is checked. Results page reflects skipped playlist tracks in unique track count, match rate, and accounting note. Added `TidalExistingPlaylist` shared type, exported `getTidalUserId` from tidal-write.
- 2026-02-16: Hardened duplicate-playlist handling after merge feedback: duplicate name matches are now informational by default (users must explicitly opt into skipping same-name playlists), matching normalizes whitespace/case, preview/results unmatched reporting now only includes playlists actually included in transfer, and `TransferPreviewUnmatchedTrack` now carries `playlistId` so reporting stays consistent when name-matched playlists are excluded.
- 2026-02-13: Added proactive OAuth token refresh: `requireProviderSession()` now detects sessions expiring within 5 minutes and refreshes them server-side using the stored refresh token. New `exchangeRefreshToken()` in oauth-client, `refreshAccessToken()` on both provider adapters, `refreshProviderSession()` + `applyRefreshedSessionCookie()` helpers. All 4 guarded API routes (playlists, liked, preview, chunk) updated to await async session check and apply refreshed cookie on response. Long transfers (5000+ tracks) now survive past the 1-hour token window.
- 2026-02-12: Fixed transfer chunk error handling: (1) server-side trim + fallback for blank/whitespace playlist names so TIDAL never rejects with "must not be blank", (2) client-side skip of remaining chunks when playlist creation fails instead of retrying each chunk individually (was causing N identical 502s per playlist), (3) preserve first error message instead of overwriting with subsequent failures, (4) show results page with failed track counts instead of full-screen error when transfer partially succeeds.
